...
<html>
    ...
    <body>
        ...
        <table class="word-table" style="margin-bottom: 10px" hidden>
            <tr>
        <th>No</th>
		<th>Tanggal</th>
		<th>Customer</th>
        ...
        <th>Karyawan</th>
            ...
            <?php
                    ...
                    <?php } ?>
            <p><b>TOTAL : </b> Rp <?php echo $sum  + $ongk; ?></p>
            </td>
        ...
        <?php
            ...
            if((!empty($tgl_awal)) && (!empty($tgl_akhir))) {
                $query1   = $this->db->query("SELECT sum(qty*harga) as tot FROM tbl_detail_order WHERE tanggal BETWEEN '$tgl_awal' AND '$tgl_akhir'");
            } else {
                $query1   = $this->db->query("SELECT sum(qty*harga) as tot FROM tbl_detail_order");
            }
            
            $results1 = $query1->result();
                foreach ($results1 as $trix)
            {
                $hgjb = ($trix->tot) + $totalongkir;
        ?>
        <table class="word-table">
            <tr>
                <td><b>Total Pendapatan Barang :</b></td>
                <td> Rp <?php echo $hgjb;?></td>
            </tr>
        </table>
        <?php } ?>
        <table class="word-table" style="margin-bottom: 10px" hidden>
        ...
        <th>Karyawan</th>
            ...
            $start = 0;
            $totalongkir = 0;
            foreach ($tbl_order_jasa as $tbl_order)
            {
                ?>
                <tr>
                <td width="10px"><?php echo ++$start ?></td>
        ...
        <table class="word-table">
            <tr>
                <td><b>Total Pendapatan Jasa :</b></td>
                <td> Rp <?php echo $hgj;?></td>
            </tr>
        </table>
        <?php } ?>
        <p class="text-left"><b>TOTAL KESELURUHAN : </b>Rp <?php echo $hgj + $hgjb ?></p>
    </body>
</html>
<script>
    window.print();
