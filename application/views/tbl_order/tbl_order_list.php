...
<div class="content-wrapper">
    <section class="content">
            ...
            <div class="col-xs-12">
                <div class="box box-warning box-solid">
                    ...
                    <div class="box-header">
                        <h3 class="box-title">KELOLA DATA RIWAYAT TRANSAKSI</h3>
                    </div>
		...
		<?php echo anchor(site_url('tbl_order/word'), '<i class="fa fa-file-word-o" aria-hidden="true"></i> Export Ms Word', 'class="btn btn-primary btn-sm"'); ?></div>
            ...
            <div class='col-md-3'>
            <form action="<?php echo site_url('tbl_order/index'); ?>" class="form-inline" method="get">
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            <?php 
                                ...
                                {
                                    ?>
                                    <a href="<?php echo site_url('tbl_order'); ?>" class="btn btn-default">Reset</a>
                                    <?php
                                }
                            ?>
		...
		<th>Action</th>
            ...
            <?php
                    ...
                    WHERE tbl_detail_order.id_order = '.$tbl_order->id_order.'');
            ...
            <?php
            $query   = $this->db->query('SELECT * FROM tbl_order LEFT JOIN tbl_user on tbl_order.id_karyawan = tbl_user.id_users WHERE id_order = '.$tbl_order->id_order.'');
            $hasil = $query->result();
			...
			<td style="text-align:center" width="200px">
                ...
                if ($this->session->userdata('id_user_level') == 2) {
                    if ($tbl_order->status == "Sedang Diproses") {
                        ...
                        echo anchor(site_url('tbl_order/read/'.$tbl_order->id_order),'<i class="fa fa-print" aria-hidden="true"></i>','class="btn btn-warning btn-sm"'); 
                    }
                    if($tbl_order->status_pembayaran == "Belum Dibayar") {
                        echo anchor(site_url('tbl_order/form_bayar/'.$tbl_order->id_order),'<i class="fa fa-upload" aria-hidden="true"></i>','class="btn btn-primary btn-sm"');  
                    }
                } else {
                    if ($tbl_order->status == "Sedang Diproses" && $tbl_order->status_pembayaran == "Sudah Dibayar") {
                        echo anchor(site_url('tbl_order/acc/'.$tbl_order->id_order),'<i class="fa fa-check-square" aria-hidden="true"></i>','class="btn btn-success btn-sm"','onclick="javasciprt: return confirm(\'Pesanan sudah diselesaikan ?\')"'); 
                        echo '  '; 
                        echo anchor(site_url('tbl_order/delete/'.$tbl_order->id_order),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'); 
